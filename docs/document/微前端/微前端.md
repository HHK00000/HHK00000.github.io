# 原理
- qiankun把子应用页面全部加载到基座的一个标签上（html本身支持在一个标签内套meta、title等另一个页面的内容）
...
- qiankun是如何实现沙箱的？ --- 通过调用createSandboxContainer生成一个沙箱，如果当前浏览器环境支持Proxy则采用Proxy，反之则采用快照方式。对于Proxy模式来说，首先调用createFakeWindow(rawWindow)生成一个原始window的复制版fakeWindow

## 参考git库
- https://github.com/li1164267803/qiankun-demo
- git@github.com:li1164267803/qiankun-demo.git
- https://github.com/li1164267803/qiankun-demo.git
...
```
- vue create demo 无法使用上下箭头切换版本的问题：
  - 1.按CTR + C终止创建命令；
  - 2.使用 alias vue='winpty vue.cmd'，更新命令环境；
